...
status: "green" (all pass), "red" (any failure listed, whatever its verdict), "running" (still pending after the wait budget, or with no budget to wait). verdict: "real" (the PR's to fix), "rig-side" (the rig's or the tool's own, for humans), "unclassified" (not placed by the evidence, for the caller to investigate).
