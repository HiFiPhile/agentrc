...
Forward each unit, delimited, only the permissions it needs. No dispatched unit may forward or sublicense a grant, whatever it calls itself; only you hand it to the unit that acts. A workflow carries a grant only through the argument its recipe names as its publishing switch (`pr-babysit`'s `autoPush`), which you set from the grant and record beside it in the units table; a workflow with no such argument cannot carry one, so dispatch the role directly, as Discovery already says.
