---
name: target-debug
description: Use when firmware — a USB device or host stack, or any code on a debug probe — misbehaves on real hardware and capture from the other end can't explain it: a test fails but usbmon shows only Submits with no Completes, the device silently NAKs, wedges, STALLs, babbles, or drops data, EP0 starves, host-side enumeration of an attached device fails, an ISR or driver state bug is suspected — and you need target-side evidence: logs over RTT/UART, GDB state dumps, a RAM ring-buffer event trace, or PC-sampling of where the core spins.
---

# target-debug — target-side capture & debugging over a debug probe

The **target** is whichever MCU runs the USB stack under test — device
role, host role, or both. Its link peer is not always a Linux PC: an MCU
host may face another board or a Linux gadget (e.g. a Raspberry Pi). Pick capture channels by which end runs Linux, not by habit:
...
channel when a Linux PC is the host; an MCU host has no usbmon on either
...
(instrument, build, flash, capture, GDB) and never stop its CI runner; the
lock protocol and the board → probe map are the project's (in TinyUSB: the
`hil` skill, see "In TinyUSB" below). Rigs and benches run many identical
probes:

- Select the probe by serial: J-Link `-SelectEmuBySN <uid>`, its GDB server
  `-select usb=<uid>`, OpenOCD `-c 'adapter serial <uid>'`.
- The J-Link device name and OpenOCD config come from the project's board
  metadata.
...
```bash
python3 ~/.claude/skills/target-debug/scripts/pc_sample.py --probe <uid> --device <JLINK_DEVICE> --elf <flashed.elf>
...
servicing normally. Native probes (ST-Link/CMSIS-DAP) are not covered by the
script yet: repeat `mdw 0xE000101C` over OpenOCD's telnet :4444 by hand.
...
both streamed a tick counter's live value plus the accessor PC (in
TinyUSB: `system_ticks` read by `tusb_time_millis_api`):
...
— no UART wiring (in TinyUSB: `LOG=2`, `LOGGER=rtt`). Stand the channel up
...
timeout 20s python3 ~/.claude/skills/rtt/scripts/rtt.py --backend jlink --probe <sn> --device <JLINK_DEVICE> > /tmp/rtt.log
...
firmware's DWARF: check the project's release configuration keeps it (in
TinyUSB `MinSizeRel` does, so `p`/struct access works on rig firmware).
...
debug-loop specifics (stack and port names for TinyUSB are in "In TinyUSB"
below):
...
timeout 30s python3 ~/.claude/skills/rtt/scripts/rtt.py --backend jlink --probe <sn> --device <dev> > /tmp/target.rtt & rtt=$!  # target (rtt skill; or ring dump after)
...
- **Halting/resetting via the probe does NOT disconnect the device**: a DWC2
  ...
  host side (in TinyUSB: the rig's `usb-kernel-recover` skill).
...
- Rig: the `hil` skill owns the lock protocol (`python3
  test/hil/helper/hil_lock.py hold <board> --reason "target debug: <bug>"`,
  `release` after; `hil_test.py` self-locks, so release before running it) and
  the board → probe map in `test/hil/tinyusb.json` (`flasher.name` = probe
  family, `flasher.uid` = probe serial); `hfp.json` on the tusb rig,
  `local.json` on a dev PC. `python3 tools/build_utils.py board-info <board>`
  prints the J-Link device and the reference Ozone project; `OPENOCD_OPTION`
  is in `hw/bsp/<family>/boards/<board>/board.cmake`.
- Build flags: `LOG=2` (`LOG=3` = per-transfer noise), `LOGGER=rtt`; the ELF
  is `cmake-build/cmake-build-<board>/<example>/<example>.elf`; `MinSizeRel`
  keeps DWARF.
- GDB names: `p/x _usbd_dev.ep_status` (device stack, `[epnum][dir]`, 1 =
  IN: busy/stalled/claimed), `p _usbh_devices[0]` (host stack per-device
  state), port state in the board's `dcd_*.c`, `watch
  xfer_status[2][1].total_len` (dwc2 names), `break dcd_int_handler`, `set
  var _usbd_dev.ep_status[2][1].busy = 0` to poke.
- Release your manual lock before `hil_test.py` — it self-locks each board and
  fails immediately on your own hold.
- The rig PC's own USB stack wedging is the `usb-kernel-recover` skill's job.
