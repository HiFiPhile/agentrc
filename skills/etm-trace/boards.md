...
selects it automatically. Timing
...
- **stm32n657nucleo** (M55, flashless): **JP2 (BOOT1) must be 1** — the app
  ...
  the stream in the startup burst (timestamp flux, not overflow) — the TinyUSB TRACE_ETM
...
- **nrf5340dk**: the interface MCU's UART1 flow control rides the trace
  ...
  SB57's SWO stub can stay — harmless at the +3 ns sample point. TinyUSB TRACE_ETM
...
- **pico2_etm_trace** (Pico 2 / RP2350 on the carrier; board `raspberry_pi_pico2`
  ...
  **Other rates need a hand-built clock** (the compile definitions: the project's
  ...
  components for pin trace not found", 0 fetches). What TinyUSB's TRACE_ETM (set by
...
- **same54_xplained**: the CM4 trace unit is clocked from **GCLK channel 47
  ...
  TinyUSB's `trace_etm_init` feeds it GCLK0. Pins PC24-28 mux to function H. The
...
- **same70_xplained**: J403 is a bottom-side bare footprint — solder the
  ...
  TinyUSB TRACE_ETM builds hold it in reset (PHY_RESET=PC10) or it drives against
